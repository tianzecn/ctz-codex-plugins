...
This capability plugin imports the skill only. It does not bundle the Firecrawl CLI or MCP server. Use the HTTP surface when neither integration is already available.

Try anonymous HTTP access first. If Firecrawl rejects the request because of IP risk controls or asks for authentication, tell the user to configure `FIRECRAWL_API_KEY` in the runtime environment and retry with `Authorization: Bearer $FIRECRAWL_API_KEY`. Never ask the user to paste the key into chat, print it, commit it, or write it into project files.
...
- HTTP: **`GET|POST https://api.firecrawl.dev/v2/search/developer`**
  ...
  Keyless where Firecrawl's risk controls allow anonymous access; send `Authorization: Bearer $FIRECRAWL_API_KEY` when authentication is required or for higher rate limits.

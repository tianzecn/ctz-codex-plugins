...
Routine public repository publication is an external write. LoopX state never
grants commit, push, or PR-creation authority by itself. Proceed only when the
user explicitly authorizes the exact repository and action in the current
task, validation passes, and the public/private boundary scan is clean. Also
stop for private or company-internal material, credentials, destructive Git
operations, production actions, or repository rules that require review.

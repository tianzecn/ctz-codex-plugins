...
TASK_NAME=$(printf '%s' "$TASK_NAME" | sed 's/[^A-Za-z0-9._-]/_/g')
if [[ -z "$TASK_NAME" || "$TASK_NAME" == "." || "$TASK_NAME" == ".." ]]; then
    echo "ERR: 无效任务名，拒绝创建或清理输出目录" >&2
    exit 2
fi
mkdir -p "$OUT_ROOT"
OUT_ROOT=$(cd "$OUT_ROOT" && pwd -P)
if [[ "$OUT_ROOT" == "/" ]]; then
    echo "ERR: 输出根目录不能是文件系统根目录" >&2
    exit 2
fi
...
safe_remove_output() {
    ...
    case "$target" in
        "$TASK_ROOT"|"$TASK_ROOT"/*) ;;
        *) echo "ERR: 拒绝清理任务目录外的路径: $target" >&2; return 1 ;;
    esac
    if [[ -z "$target" || "$target" == "/" || "$target" == "$OUT_ROOT" ]]; then
        echo "ERR: 拒绝清理不安全路径: $target" >&2
        return 1
    fi
    rm -rf -- "$target"
}
...
if [[ "$SKIP_APKTOOL" != "true" ]]; then
    safe_remove_output "$APKTOOL_OUT"
    echo "=== apktool 解包 ==="

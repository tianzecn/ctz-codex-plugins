...
function Get-SafeTaskName {
    ...
    if ([string]::IsNullOrWhiteSpace($safe) -or $safe -eq '.' -or $safe -eq '..') {
        throw 'Invalid task name: refusing to create or clean an unsafe output path.'
    }
    return $safe
}
...
$rawTaskName = if ($Name) { $Name } else { Get-SafeName -PathValue $ApkPath }
$taskName = Get-SafeTaskName -Value $rawTaskName
$rawOutRoot = if ([string]::IsNullOrWhiteSpace($OutRoot)) { Get-DefaultOutRoot -ApkFilePath $ApkPath } else { $OutRoot }
$resolvedOutRoot = [System.IO.Path]::GetFullPath($rawOutRoot)
if ($resolvedOutRoot -eq [System.IO.Path]::GetPathRoot($resolvedOutRoot)) {
    throw 'Output root cannot be a filesystem root.'
}
$taskRoot = [System.IO.Path]::GetFullPath((Join-Path $resolvedOutRoot $taskName))
$trimChars = [char[]]@([System.IO.Path]::DirectorySeparatorChar, [System.IO.Path]::AltDirectorySeparatorChar)
$outRootPrefix = $resolvedOutRoot.TrimEnd($trimChars) + [System.IO.Path]::DirectorySeparatorChar
if (-not $taskRoot.StartsWith($outRootPrefix, [System.StringComparison]::OrdinalIgnoreCase)) {
    throw "Unsafe task output path: $taskRoot"
}

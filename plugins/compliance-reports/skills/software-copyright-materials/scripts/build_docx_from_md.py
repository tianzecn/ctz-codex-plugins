...
def docx_checks(skill_dir: Path, outputs: list[Path]) -> list[str]:
    ...
    cli_dir = skill_dir / "vendor/docx-toolkit/scripts/dotnet/DocxToolkit.Cli/bin/Debug"
    built_cli = any(
        (cli_dir / framework / "DocxToolkit.Cli.dll").exists()
        for framework in ("net10.0", "net8.0")
    )
    if env_script.exists() and built_cli:
    ...
    elif env_script.exists():
        notes.append(
            "DOCX env: NOT READY\n\n"
            "The environment check skipped NuGet restore/build. Run the bundled setup "
            "script only after explicit user approval."
        )
    else:

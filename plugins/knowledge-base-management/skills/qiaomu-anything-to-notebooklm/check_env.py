...
def check_mcp_config():
    ...
    if not config_path.exists():
        print_status("error", f"未找到 Codex 配置文件: {config_path}")
        return False

    try:
        config_text = config_path.read_text(encoding="utf-8")
        if "[mcp_servers.weixin-reader]" in config_text or "weixin-reader" in config_text:
...
def main():
    ...
    if passed < total:
        ...
        print("  2. 配置 MCP：编辑 ~/.codex/config.toml")

...
MCP 服务器已安装在：`~/.codex/skills/qiaomu-anything-to-notebooklm/wexin-read-mcp/`

**配置 MCP**（需要手动添加到 Codex 配置文件）：

**macOS**: 编辑 `~/.codex/config.toml`

```toml
[mcp_servers.weixin-reader]
command = "python"
args = [ "/absolute/path/to/qiaomu-anything-to-notebooklm/wexin-read-mcp/src/server.py" ]
enabled = true
```
...
- 调用 `python3 ~/.codex/skills/qiaomu-anything-to-notebooklm/scripts/get_podcast_transcript.py <URL>`
- 脚本自动执行：创建链接笔记 → 等待转写 → 获取全文 → 保存 TXT
- 返回 TXT 路径和标题
- 依赖：Get笔记 API Key（环境变量 `GETNOTE_API_KEY`、`GETNOTE_CLIENT_ID`）+ Web Token（`~/.codex/skills/getnote/tokens.json`）

**X/Twitter 帖子**：
- 通过内置代理级联抓取推文内容（r.jina.ai → defuddle.md → agent-fetch）
- 调用 `bash ~/.codex/skills/qiaomu-anything-to-notebooklm/scripts/fetch_url.sh "https://x.com/..."` 获取 Markdown 内容
- 自动处理 X 登录墙和错误页面
...
```bash
# 仅深度分析
python ~/.codex/skills/qiaomu-anything-to-notebooklm/main.py \
...
```bash
# 测试 MCP 服务器
python ~/.codex/skills/qiaomu-anything-to-notebooklm/wexin-read-mcp/src/server.py

# 如果报错，检查依赖
cd ~/.codex/skills/qiaomu-anything-to-notebooklm/wexin-read-mcp

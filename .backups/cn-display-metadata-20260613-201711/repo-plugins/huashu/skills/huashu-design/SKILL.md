...
name: huashu-design
description: |
  花叔Design：用 HTML 做高保真原型、交互 Demo、幻灯片、动画、设计变体探索、专家评审和带解说的视频式视觉作品。
  适用于做原型、HTML 演示、动画 Demo、设计变体、UI mockup、App 原型、导出 MP4/GIF、配音动画、概念解释视频等请求。
  强制先核对事实和资产，并根据任务切换 UX 设计师、动画师、幻灯片设计师或原型师工作流，避免通用 AI 味设计。
---
...
**真实素材优先原则**（涉及用户本人/产品时）：
1. 先查用户配置的**私有 memory 路径**下的 `personal-asset-index.json`（Codex 默认在 `~/.codex/memories/`；其他 agent 按其自身约定）
